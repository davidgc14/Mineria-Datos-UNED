...
El objetivo principal de este trabajo es aplicar diferentes algoritmos de árboles de decisión para clasificar los tumores como benignos o malignos, utilizando las características morfológicas extraídas de las imágenes. A lo largo del trabajo, se explorarán diversas técnicas de preprocesamiento, selección de características y modelado, con el fin de optimizar el rendimiento de los modelos y proporcionar una comprensión más profunda del conjunto de datos.
...
Al igual que en el caso anterior, el orden mostrado es el asociado a la suma de los resultados obtenidos en ambos modelos. Sin embargo, se observa aquí en la Figura~\ref{fig:fselection-Perm} una variabilidad mucho más alta en cuanto a los resultados obtenidos por los dos modelos. Por ejemplo, destaca que la primera posición la ocupe la variable \textit{worst radius}, cuando para el modelo \textit{Random Forest} ocupaba la posición 10. Otro caso muy visual es el de la variable \textit{mean smoothness}, donde ambos modelos difieren por completo en su impacto en el rendimiento del modelo.
...
\begin{enumerate}
    \item \textbf{Random Forest:} Por obtener el mejor rendimiento general (Accuracy y F1-Score) y mostrar un buen equilibrio entre precisión y exhaustividad, además de una estabilidad razonable. Es el algoritmo central de esta práctica y su rendimiento justifica plenamente su inclusión.
    \item \textbf{Gradient Boosting:} Por su rendimiento casi idéntico al de Random Forest, su excelente estabilidad (bajas desviaciones estándar) y por ser un representante clave de la familia de algoritmos de Boosting, lo que permite una comparación directa y profunda entre las dos principales estrategias de ensembles basados en árboles.
    \item \textbf{Bagging Classifier:} Aunque ligeramente inferior a los dos anteriores, su rendimiento sigue siendo muy alto y superior al resto de modelos. Incluirlo permite analizar directamente el impacto de la modificación introducida por Random Forest (selección aleatoria de variables) y evaluar si la optimización de hiperparámetros puede acercar su rendimiento a los de \textit{Random Forest} y \textit{Gradient Boosting}.
\end{enumerate}
...
La evaluación final en el conjunto de test revela un rendimiento muy similar y alto para los tres modelos optimizados. Tanto \textit{Gradient Boosting} como \textit{Bagging Classifier} obtienen un F1-score idéntico de 0.9392, superando ligeramente a \textit{Random Forest}.

Al examinar los informes de clasificación y las matrices de confusión, observamos que \textit{Gradient Boosting} y \textit{Bagging} no solo alcanzan el mismo F1-score global, sino que también presentan métricas idénticas por clase y la misma matriz de confusión. Ambos clasifican erróneamente 6 instancias benignas como malignas, y solo un falso negativo. Esto se traduce en una precisión muy alta para la clase benignas y un recall muy alto para la clase malignas.
