...
Continuando con \texttt{PCA} de la librería \texttt{scikit-learn}, se han considerado los siguientes parametros: \texttt{n\_components=5} (para conservar las 5 variables y estudiar su varianza) y \texttt{svd\_solver="full"} (para realizar la descomposición completa de la matriz de covarianza\footnote{Se utiliza una descomposición completa para no perder precisión en el cálculo, al ser un dataset de tamaño pequeño. Así se recomienda en la propia documentación de la librería \cite{scikitlearnDoc}.}).
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        \hline
        \textbf{E. Variance} & 0.27        & 0.21        & 0.19        & 0.18        & 0.14        \\
        \hline
    \end{tabular}
    \label{tab:pca}
\end{table}

\begin{wrapfigure}{r}{0.4\textwidth} % 'r' para la derecha, el ancho de la figura
    \centering
    \includegraphics[width=0.45\textwidth]{images/loadings.png}
    \caption{Loadings de cada variable en cada componente principal.}
\end{wrapfigure}
...
Por último, para el método de RFE-SVM, se ha utilizado la implementación \texttt{RFE} de \texttt{scikit-learn} con un modelo base SVM de kernel lineal (\texttt{SVC(kernel=`linear')}). Los hiperparámetros seleccionados incluyen \texttt{n\_features\_to\_select=1} (para obtener un ranking completo) y \texttt{step=1} (eliminar una variable por iteración), siguiendo las recomendaciones de \cite{guyon2002} para garantizar un proceso conservador que minimice la pérdida prematura de variables relevantes.
